...
C4Context
    ...
    System(crm, "CRM System", "Manages contacts, appointments, staff scheduling, automated reminders, and AI-powered email intake")

    System_Ext(email, "Email Providers", "SendGrid/Mailgun for receiving and sending emails, webhooks for inbound email")
    ...
    System_Ext(ai, "AI Provider", "OpenAI/Anthropic for email analysis, summarization, and recommendations")

    Rel(staff, crm, "Manages appointments, reviews AI intakes", "HTTPS/JSON")
    Rel(patient, crm, "Books appointments, views history", "HTTPS/JSON")
    Rel(admin, crm, "Manages staff, configures system", "HTTPS/JSON")

    Rel(email, crm, "Sends inbound emails", "Webhook/HTTPS")
    ...
    Rel(crm, ai, "Analyzes emails, generates summaries", "REST API")
...
C4Container
    ...
    Container_Boundary(crm_boundary, "CRM System") {
        Container(web_app, "Web Application", "FastAPI", "REST API for CRM functionality, email intake, webhooks")
        Container(ai_orchestrator, "AI Orchestrator", "OpenAI/Anthropic", "Email analysis, summarization, task/deal recommendations")
        Container(scheduler, "Background Scheduler", "APScheduler", "Processes scheduled tasks like sending reminders")
        ContainerDb(database, "Database", "PostgreSQL/SQLite", "Stores contacts, appointments, staff, availability, reminders, email intakes")
    }

    System_Ext(email_providers, "Email Providers", "SendGrid/Mailgun webhooks for inbound email")
    System_Ext(email_service, "Email Service", "SendGrid/SES for outbound")
    System_Ext(sms_service, "SMS Service", "Twilio")
    System_Ext(calendar_client, "Calendar Client", "Google/Outlook")
    System_Ext(ai_provider, "AI Provider", "OpenAI/Anthropic API")

    Rel(user, web_app, "Uses", "HTTPS/JSON")
    Rel(email_providers, web_app, "Sends inbound emails", "Webhook/HTTPS")
    Rel(web_app, ai_orchestrator, "Analyze emails", "Function calls")
    Rel(ai_orchestrator, ai_provider, "Generate summaries", "REST API")
...
**Containers:**
1. **Web Application (FastAPI)**: REST API, business logic, request handling, webhook receivers
2. **AI Orchestrator**: Email analysis, intent classification, task/deal recommendations
3. **Background Scheduler (APScheduler)**: Automated reminder processing
4. **Database (PostgreSQL/SQLite)**: Persistent storage including email intake records
...
- **OpenAI/Anthropic**: LLM providers for AI analysis
- **BeautifulSoup4**: HTML email cleaning
...
C4Component
    ...
    Container_Boundary(web_app, "Web Application (FastAPI)") {
        ...
        Component(router_email_intake, "Email Intake Router", "FastAPI Router", "Handles email processing and review")
        Component(router_webhooks, "Webhook Router", "FastAPI Router", "Receives emails from providers")

        Component(webhook_sendgrid, "SendGrid Parser", "Webhook Adapter", "Parses SendGrid Inbound Parse")
        Component(webhook_mailgun, "Mailgun Parser", "Webhook Adapter", "Parses Mailgun with HMAC validation")
        Component(webhook_generic, "Generic Parser", "Webhook Adapter", "Generic format with token auth")

        Component(email_normalizer, "Email Normalizer", "Service", "HTML cleaning, quote removal")
        Component(crm_context, "CRM Context Lookup", "Service", "Contact and interaction lookup")
        Component(ai_intake, "AI Intake Engine", "Service", "LLM analysis and recommendations")
        Component(intake_repo, "Intake Repository", "Repository", "Email intake persistence")
    ...
    Person(user, "User")
    System_Ext(email_provider, "Email Providers")
    System_Ext(ai_provider, "AI Provider")
    ...
    Rel(user, router_email_intake, "Review intakes", "HTTPS/JSON")

    Rel(email_provider, router_webhooks, "Send emails", "Webhook/HTTPS")
    Rel(router_webhooks, webhook_sendgrid, "Parse SendGrid")
    Rel(router_webhooks, webhook_mailgun, "Parse Mailgun")
    Rel(router_webhooks, webhook_generic, "Parse generic")

    Rel(router_webhooks, email_normalizer, "Normalize email")
    Rel(router_email_intake, email_normalizer, "Normalize email")
    Rel(email_normalizer, crm_context, "Lookup context")
    Rel(crm_context, ai_intake, "Analyze with context")
    Rel(ai_intake, ai_provider, "Call LLM API")
    Rel(ai_intake, intake_repo, "Save intake")
...
- **Email Intake Router**: `/api/v1/email-intakes/*` - Manual email processing and review
- **Webhook Router**: `/api/v1/webhooks/email/*` - Automatic email reception

### Email Intake Components (Clean Architecture - Phase 6)
- **Webhook Parsers**: Provider-specific email parsing (SendGrid, Mailgun, Generic)
- **Email Normalizer**: HTML cleaning, quote removal, metadata extraction
- **CRM Context Lookup**: Contact and interaction history lookup
- **AI Intake Engine**: LLM-powered analysis, intent classification, recommendations
- **Intake Repository**: Domain entity persistence with JSON serialization
...
- **SQLAlchemy Models**: Entities (Contact, Staff, Appointment, EmailIntake, etc.)
